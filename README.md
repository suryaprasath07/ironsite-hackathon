...
SpatialFlow is an AI-powered construction site logistics tool that reads your site plan image and project schedule to recommend optimal temporary zone placements, worker paths, and delivery sequences for each week of your project. It uses Claude (Anthropic) to analyse the real layout of your site and generate structured spatial decisions — not redesigns, but week-by-week temporary positioning of materials, equipment, and people. When a disruption hits, it instantly replans the entire schedule to eliminate idle days.
...
| **Delivery Sequence** | Orders deliveries by day and staging zone to prevent congestion and truck dwell time |
...
All intelligence lives in four Flask endpoints. Each one constructs a precise prompt, calls Claude via the Anthropic SDK, and returns structured JSON. Claude is never asked to generate UI or explain itself — only to return valid JSON arrays or short text answers.

- `/parse-schedule` — Takes raw schedule text in any format and returns a structured JSON array of weeks with `activity`, `trades`, and `materials` fields. This powers the week slider in the UI.
- `/generate-layout` — The core endpoint. Sends the current week context, full schedule, and optionally the site plan image to Claude. Returns zone recommendations, worker paths, delivery sequence, materials table, and a key spatial insight. If an image is provided, Claude references actual structural features in its recommendations rather than generic directions.
- `/query` — Answers a freeform spatial question using the current week context, schedule excerpt, and optional image. Returns bullet-pointed text under 200 words.
...
- **Temporary-only framing** — Every layout prompt tells Claude it is not redesigning the site — only recommending where to place temporary items this week. This prevents Claude from suggesting structural changes and keeps every recommendation immediately actionable.
...
- Claude with a site plan image produces significantly more accurate zone placement than text-only prompts. With an image it outputs things like "stage rebar adjacent to the north boundary wall, clear of the existing drainage channel" — without an image it outputs "stage rebar in the north area of the site."
- Telling Claude the current week number, active trades, and materials needed *before* the full schedule produces better recommendations than burying that context inside a long schedule block. Prompt order matters.
...
4. Click **Generate Layout Plan**
   ...
   - Delivery order appears in the **Delivery Sequence** tab
